import {commentContents} from "../../apis/comment"
...
// 상태줘서 사용 할 것
function WittyComment({newData,commentSwitch,setCommentSwitch,myWitty}){
  const [comment,setComment] = useState([]);
  const [page,setPage] = useState(0);
  const commentMyWitty = myWitty;
  ...
  const setCommentBox = (result,refresh)=>{
    ...
    if(arrayData.length === 0){
      ...
      arrayData.map((data)=>{
        userCommentBox.push(
            <V2WittyCommnetAdd data={data} commentSwitch={commentSwitch} setCommentSwitch={setCommentSwitch} newData={newData} commentMyWitty={commentMyWitty} key={data.id} />
        )
      })
      refresh === 3
      ?setComment(userCommentBox,[...comment])
